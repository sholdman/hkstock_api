...
const scrapEtnetQuotePage = async (stockCode) => {
 ...
 const scrapEtnetTop20 = async (type) => {
     const etnetUrl = "http://www.etnet.com.hk/www/tc/stocks/realtime/top20.php?subtype=" + type;
     const html = await fetchHtml(etnetUrl);
     const selector = cheerio.load(html);
...
//     var selector = await scrapEtnet(stockCode);
